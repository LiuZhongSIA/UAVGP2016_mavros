...
int main(int argc, char **argv)
{
    ...
    // 姿态期望，主要是航向
    auto quat = mavros::ftf::quaternion_from_rpy(0.0, 0.0, 1.5708);
    pose_a.pose.orientation.x = quat.x(); //机头将指向北
    ...
    pose_c.pose.orientation.x = quat.x();
    pose_c.pose.orientation.y = quat.y();
    pose_c.pose.orientation.z = quat.z();
    pose_c.pose.orientation.w = quat.w();
    // 降落参数设置landing_cmd
    ...
    tol_client = nh.serviceClient<mavros_msgs::CommandTOL>("mavros/cmd/land");
    // 用于消息订阅
